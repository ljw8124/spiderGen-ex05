...
ASlideView.prototype.addItems = function(urlArr, dataArr, isPrepend, asyncCallback)
{
	...
	for(var i=0; i<urlArr.length; i++)
	{
		if(asyncCallback)
		{
			this.addItem(urlArr[i], [ dataArr[i] ], isPrepend, function(items, inx)
			{
				//newItems.push(items[0]);
				newItems[inx] = items[0];
...
ASlideView.prototype.addItem = function(url, dataArray, isPrepend, asyncCallback, turnback)
{
	...
	for(var i=0; i<dataArray.length; i++)
	{
		...
		if(asyncCallback) 
		{
			AView.createView(item, url, this, null, null, null, function(_aview)
			{
				...
				{
					asyncCallback(newItems, turnback);
				}
			});
		}
		else 
		{
			...
			else aview = AView.createView(item, url, this, null, null, null, null, this.lastLoadedHtml);
		}
