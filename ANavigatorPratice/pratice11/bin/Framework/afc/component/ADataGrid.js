...
ADataGrid.prototype._gridClickManage = function(acomp, info, e)
{
	...
	var pos = acomp.indexOfCell(cell),
		...
		colInx = addVal + pos[1],
		parentArr = this.showArr2[rowInx];
...
ADataGrid.prototype._onScrollY = function(acomp, info, e)
{
	...
	if(newInx != this.dataInx) 
	{
		...
		// 이 코드는 성능상 좋지 않지만 머지된 그리드의 스크롤을 위해서 필요하다.
		// ★ 향후 resetInitRow 함수의 성능 개선 필요. 
		this.resetInitRow();
...
//	★ 성능 개선을 위해.... removeAll, addRow 대신에... 한번에 셋팅되도록 하는 방안 구상하기.
ADataGrid.prototype.resetInitRow = function()
{
	//반대로 로우가 삭제되어 0이 되는 경우도 있으므로 이 비교를 하면 안됨.
	//if(this.showArr2.length<1) return;
	
	var cnt = Math.min(this.scrollBarV.getCountPerArea(), this.showArr2.length);
	
	//이 코드는 머지된 그리드가 스크롤 될 때, 호출되는 resetInitRow 에서 참이되므로 추가되면 안됨.
...
ADataGrid.prototype.renderData = function()
{
	...
	for(; i<end; i++)
	{
		if(this.pivotGrid) this.pivotGrid.setRowByObj(this, j, this.showArr2[i]);
		
		this.grid.setRowByObj(this, j++, this.showArr2[i], this.startCol, this.endCol, dataOffset);
	}
};
...
ADataGrid.prototype._buttonClick = function(acomp, info, e)
{
	if(this.delegator) 
	{
