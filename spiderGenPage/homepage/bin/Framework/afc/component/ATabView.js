...
//	if(this.tabArea.css('visibility')=='hidden') this.hideTabArea();
	...
	{
		if(!this.historyInfo) this.enableHistory(true);
...
ATabView.prototype.tabChangeManage = function(tabEle)
{
	...
	*/
	...
	if(!tabEle.content.view) //addtab 시점의 oneshot 으로 옮겨짐.
	{
		...
		//비동기 로드
		if(tabEle.asyncCallback)
		{
			var thisObj = this;
			
			this.loadTabContent(tabEle, null, function(_aview)
			{
				if(typeof(tabEle.asyncCallback)=='function') tabEle.asyncCallback(_aview);
		...
		{
			this.loadTabContent(tabEle, null, false);
			this.activeTab(this.oldTab, this.selectedTab, true);
		}
	}
