...
AGrid.CONTEXT = 
{
    tag: '<div data-base="AGrid" data-class="AGrid" data-fullrow-select="true" data-selectable="true" data-clear-rowtmpl="true" class="AGrid-Style">\
...
AGrid.prototype.addRows = function(infoArr2, rowData2)
{
	var rowArr = [], row;
...
AGrid.prototype._addCell = function(cellArr)
{
	...
	if(!this.isDev() && $(cellArr)[0].isHeader) return;
...
AGrid.prototype.setColumnWidth = function(colIdx, width)
{
...
{
	var thisObj = this;
	var $cells = $(this.getRow(rowInx)).children(), text;
	...
	for(var i=start; i<=end; i++)
	{
		...
		if(!obj) continue;
		text = obj.maskdata||obj.text||this.$rowTmpl.children().eq(i).text();
		...
		if(obj.type)
		{
			if(obj.type == 'button')
			{
				//cell.innerHTML = '<button type="button">'+ (text == undefined? '': text) +'</button>';
				
				//datagrid_button 클래스는 스타일을 추가하거나 그룹으로 얻기 위해 추가
				cell.innerHTML = '<button class="datagrid_button">' + text + '</button>';
				
				//계속 이벤트가 중첩되어 성능에 문제를 야기하므로 주석처리 다른 방식으로 변경해야함
				//cell.children[0].addEventListener('click', function(e){ dataGrid._buttonClick(this, rowData, e); });
			}
			/*
...
//----------------------------------------------------------------------
//	cellEles 는 element 를 담고 있는 배열이거나 jQuery 집합 객체이다.
//	그룹지어야 할 cell들을 배열이나 jQuery 집합으로 모아서 넘긴다.
//	cell 이 추가된 후에 공통적으로 처리해야 할 것들...
AGrid.prototype.cellProcess = function(cellEles)
{
	//현재는 사라져서 하는 일이 없음.
	//추후에 필요없어지면 함수 제거
};
...
AGrid.prototype.updatePosition = function(pWidth, pHeight)
{
	...
	if(this.option.isColumnResize) this._updateBarPos();
};
...
{
	var tbody, rows, switching, i, j, x, y, shouldSwitch, ret,
	...
	while (switching) 
	{
		...
		for (i = 0; i < this.getRowCount()-rsLen; i=i+rsLen) 
		{
			//start by saying there should be no switching:
			shouldSwitch = false;
			
			x = this.getCellText(i+plusRowIndex, colInx);
			y = this.getCellText(i+rsLen+plusRowIndex, colInx);
			
			ret = this.sortFunc(x, y, isAsc);	// isAsc ? (x > y) : (x < y);
		...
		if (shouldSwitch) 
		{
			...
			for(j=0; j<rsLen; j++)
			{
				tbody.insertBefore(this.getRow(i+rsLen+rsLen-1), this.getRow(i));
			}
			switching = true;
		}
	}
...
AGrid.prototype.columnResizable = function()
{
	//이 함수를 직접 호출할 수도 있으므로 변수를 셋팅해 둔다.
	this.option.isColumnResize = true;
	
	this.$headCols = this.showThead.find('td');
	this.resizeBars = [];

	var thisObj = this, pos = 0, end = this.$headCols.length - 1;
	
	this.$headCols.each(function(i)
	{
		if(i==end) return;
		...
		thisObj.resizeBars.push($sizeBar);

		thisObj._resizeProcess($sizeBar, i);
	});
	
	//리사이즈를 활성화 시키면 마지막 컬럼에 셋팅된 값을 지워야 정상 작동한다.
	this.setColumnWidth(end, '');

};

AGrid.prototype._resizeProcess = function($sizeBar, inx)
{
	var thisObj = this;
	...
	{ 
		...
		stop: function( event, ui ) 
		{
			...
			thisObj._colculResizeWidth(inx, ui.position.left);
...
AGrid.prototype._colculResizeWidth = function(inx, newLeft)
{
	var preLeft = 0;
	...
	{
		moveWidth = moveWidth/this.headerTable.width() * 100;
		this.setColumnWidth(inx, moveWidth+'%');
	}
	else
	{
		this.setColumnWidth(inx, moveWidth+'px');
	}
};
