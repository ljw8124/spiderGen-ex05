...
//
//	turnback 은 실제로 asyncCallback 이 참인 경우(비동기방식)에 사용하는 변수, 즉 비동기인 경우 다시 되돌려주는 변수
//	asyncCallback 변수를 거짓으로 넘기면서 turnback 에 이미 로드된 html 을 넘기면 loadHtml 를 호출하지 않고 view 를 생성한다.
//	asyncCallback 은 관련된 js 파일도 비동기로 로드되게 한다.
...
AView.createView = function(item, url, owner, eventListener, skipUpdatePos, skipActiveDone, asyncCallback, turnback)
{
	...
	else afc.loadHtml(item, url, _loadHelper, null, null, Boolean(asyncCallback));//, new RegExp(searchValue, 'g'), compIdPrefix);
	...
	function _loadHelper(retHtml)
	{
		...
		if(PROJECT_OPTION.build.frwLoadOption=='inTimeLoad')// && !isAView) 
		{
			...
			if(classMap)
			{
				...
				for(p in classMap)
				{
					...
					{
						afc.loadScript('Framework/' + p + '/component/' + arr[i] + '.js', asyncCallback);
						afc.loadScript('Framework/' + p + '/event/' + arr[i] + 'Event.js', asyncCallback);
					}
				}
			}
		}
		...
		if(!isAView && url) 
		{
			...
			afc.loadScript( url.substring(0, url.lastIndexOf(".")) + '.js', asyncCallback, AView.enforceClsLoad);	//true, 무조건 강제 로드
		}
		//-------------------------------------------------------------------------		
		
		//위에서 동적으로 로드한 스크립트가 모두 로드된 후에 진행되도록
		//위의 스크립트 내부에서 호출된 afc.loadScript(); 까지 모두 로드되어야 호출된다.
		afc.scriptReady(function()
		{
			...
			rootView.isAsyncQryLoad = Boolean(asyncCallback);
			...
			afc.queryReady(function()
			{
				//rmate component load check
				if(!aview._rMateManage(skipUpdatePos, skipActiveDone))	//알메이트 컴포넌트가 없으면 
				{
					setTimeout(function()
					{
						if(aview.isValid())
...
AView.prototype.init = function(context, evtListener)
{
	...
	{
		afc.loadScript('Framework/afc/component/ALayout.js');
	}
...
AView.prototype._initCrudComponent = function() 
{
	...
	{
		this._findChildCrudComp(this.getChildren()[i]);
	}
...
AView.prototype._rMateManage = function(skipUpdatePos, skipActiveDone) 
{
...
{
	afc.loadScript('Framework/afc/library/ScrollArrow.js');
...
AView.prototype.enableScrlManagerX = function()
{
	...
	afc.loadScript("Framework/afc/library/ScrollManager.js");	//차후 주석 달기, 라이브러리에서 추가하기
...
AView.prototype.loadView = function(url, asyncCallback, turnback)
{
	...
	{
		if(turnback) this.ldView = AView.createView($item[0], url, this, null, null, null, false, turnback);
...
AView.prototype.loadContainer = function(viewUrl, cntrId, data, cntrClass)
{
