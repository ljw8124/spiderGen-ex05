...
ADropBox.prototype.openBox = function()
{
	...
	if(this.option.inParent)
	{
		...
		if(cntr)
		{
			...
			var cntrOfs = cntr.get$ele().offset();
			pos.left -= cntrOfs.left;
			pos.top -= cntrOfs.top;
		}
	}
	
	var boxWidth = this.$ele.outerWidth() - 2;
	
	//if(!this.isDev() && !afc.isSimulator && window.AppManager) AppManager.bringToFront(true);
	
	this.listPopup.popup(pos.left, pos.top, boxWidth, boxHeight, function()
	{
		/*
